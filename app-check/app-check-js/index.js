...
exports.createToken = functions.region("europe-west1").https.onRequest((req, res) => {
    firebaseAdmin.appCheck().createToken(req.query.appId)
        .then((token) => {
            return res.send({expiresAt: Math.floor(Date.now() / 1000) + 60 * 60, token: token})
        })
        .catch((err) => {
...
exports.verifyToken = functions.region("europe-west1").https.onRequest((req, res) => {
    firebaseAdmin.appCheck().verifyToken(req.header("X-Firebase-AppCheck"))
        .then((tokenRes) => {
            return res.status(200).send(tokenRes.token)
        })
        .catch((err) => {
