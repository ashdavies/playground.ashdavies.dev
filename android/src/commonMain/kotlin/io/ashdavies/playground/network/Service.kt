...
inline fun <T : Any, reified R> serviceOperator(httpClient: HttpClient, crossinline urlString: (String) -> String) =
    ReadOnlyProperty<Service, ServiceOperator<T, R>> { _, property ->
        serviceOperator(httpClient, urlString(property.name))
    }

inline fun <T : Any, reified R> serviceOperator(httpClient: HttpClient, urlString: String) =
