...
internal fun GalleryUi(state: GalleryScreenState, modifier: Modifier = Modifier) {
    ...
    BottomSheetScaffold(
        ...
        floatingActionButton = {
            GalleryActionButton(
                onClick = { eventSink(GalleryScreenEvent.Capture.Request) },
        ...
        when {
            state.itemList.isEmpty() -> {
                ...
                GalleryGrid(
                    ...
                    onExpand = { eventSink(GalleryScreenEvent.Selection.Expand(it)) },
                    onSelect = { eventSink(GalleryScreenEvent.Selection.Toggle(it)) },
                    modifier = Modifier
        ...
        if (state.showCapture) {
            ...
            ImageCapture(
                onResult = {
                    when (it) {
                        is Path -> eventSink(
                            GalleryScreenEvent.Capture.Result(
                                target,
                            ),
                        )

                        null -> eventSink(GalleryScreenEvent.Capture.Cancel)
                    }
                },
            )
        }
...
internal fun GalleryGrid(
    itemList: ImmutableList<GalleryScreenState.StandardItem>,
    onExpand: (Int) -> Unit,
    onSelect: (Int) -> Unit,
    modifier: Modifier = Modifier,
